...
def test_complete_chain_up(tmp_path: Path) -> None:
    ...
    for line in lines:
        row = json.loads(line)
        assert row["correlation_id"] == "corr-r3a"
...
def test_fact_jsonl_roundtrip(tmp_path: Path) -> None:
    ...
    sink = JsonlFactSink(path)
    fact = make_fact(
        fact_type="test_fact",
        ts=datetime(2026, 7, 16, 12, 0, 0, tzinfo=timezone.utc),
        run_id=RunId("r"),
        correlation_id=CorrelationId("c"),
        payload={"price": Decimal("0.5"), "when": datetime(2026, 7, 16, 12, 0, 0, tzinfo=timezone.utc)},
    )
    sink.append(fact)
    sink.flush()
    sink.close()
    loaded = json.loads(path.read_text(encoding="utf-8").strip())
    assert loaded == fact_to_jsonable(fact)
