...
from tyrex_pm.market_data.book_feed import COLD_GAP_MAX_S, BindingFeed, BookFeedSupervisor
...
def _seed_ready(
    store: MarketStateStore, disp: EventDispatcher, token: str, ask: str, bid: str
) -> None:
    disp.publish(
        BookSnapshotReceived(
...
async def test_rollover_promote_preserves_warm_books_and_cold_gap_budget(tmp_path) -> None:
    ...
    feed = BindingFeed(binding=prepared, store=store, dispatcher=disp, rest_fetcher=fetcher)
    for tok in (UP, DOWN):
        store.apply_rest_snapshot(
            fetcher(tok).book,
            ts_received=TS,
...
def test_revalidation_version_bump_soft_and_hard_invalidators() -> None:
    ...
    rolled = revalidate_plan_against_book_view(plan, view3, active_binding_id="other-binding")
