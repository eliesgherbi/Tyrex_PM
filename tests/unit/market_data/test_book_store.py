...
def test_seed_trading_parameters_fill_if_missing_and_survive_ws_snapshot() -> None:
    store = MarketStateStore()
    disp = EventDispatcher()
    store.attach(disp)
    assert store.seed_trading_parameters(
        UP.value,
    ...
    seeded = store.get(UP)
    assert seeded.tick_size == Decimal("0.01")
    assert seeded.min_order_size == Decimal("5")
    assert seeded.binding_id == "bind-1"
    assert seeded.book_version == 0
    # Second seed must not overwrite an established tick.
    assert not store.seed_trading_parameters(UP.value, tick_size=Decimal("0.001"))
    assert store.get(UP).tick_size == Decimal("0.01")
    ...
    assert after.tick_size == Decimal("0.01")
    assert after.min_order_size == Decimal("5")


def test_binding_tick_size_round_trips_through_persistence(tmp_path: Path) -> None:
    record = _binding()
    record = MarketBindingRecord(
        binding_id=record.binding_id,
        window_slug=record.window_slug,
        ...
        role=record.role,
        role_epoch=record.role_epoch,
        outcome_semantics=record.outcome_semantics,
        resolved_at=record.resolved_at,
        tick_size=Decimal("0.01"),
        min_order_size=Decimal("5"),
    )
    path = tmp_path / "bindings.json"
    persist_bindings(path, [record])
    loaded = load_bindings(path)[0]
    assert loaded.tick_size == Decimal("0.01")
    assert loaded.min_order_size == Decimal("5")
    promoted = loaded.with_role(BindingLifecycleRole.ACTIVE, role_epoch=2)
    assert promoted.tick_size == Decimal("0.01")
