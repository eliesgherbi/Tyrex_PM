...
@pytest.mark.asyncio
async def test_rest_bootstrap_does_not_block_asyncio_loop_on_slow_network() -> None:
    store = MarketStateStore()
    dispatcher = EventDispatcher()
    store.attach(dispatcher)

    def slow_fetcher(token_id: str):
        time.sleep(0.03)
        return normalize_rest_book_payload(
            {
                "asset_id": token_id,
    ...
    feed = BindingFeed(
        ...
        rest_fetcher=slow_fetcher,
    )
    task = asyncio.create_task(feed.bootstrap_rest(mark_ready=True))
