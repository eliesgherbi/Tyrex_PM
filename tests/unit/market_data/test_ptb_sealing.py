...
def test_exact_selection_clears_stale_absence_blockers() -> None:
    ...
    eng.ingest_chainlink(market_id=MID, window_id=WID, tick=_tick("100.5", source_ts=TS0))
...
def test_exact_boundary_association_and_seal() -> None:
    ...
    assert eng.get_window(MID, WID).selected_candidate.rule_id is BoundaryRuleId.EXACT_AT_START
    ...
    assert sealed.boundary_lag_ms == 50
...
def test_attestation_mismatch_still_allows_seal_without_require_match() -> None:
    eng = PtbCaptureEngine(
        attestation_port=FixturePtbAttestationProvider(by_window={WID: ("200", "fixture", {})})
    )
    eng.open_window(
        market_id=MID, window_id=WID, event_start=TS0, event_end=TS0 + timedelta(minutes=5)
    )
