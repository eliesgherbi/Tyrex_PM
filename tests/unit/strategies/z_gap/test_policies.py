...
def test_leg_selection_up_down_and_less_probable() -> None:
    ...
    up = value_entry_leg(
        model=model,
        leg=ZGapLeg.UP,
        book=LegBookView(ask=Decimal("0.69"), ask_depth=Decimal("20"), bid=None, bid_depth=None),
        config=cfg,
    )
    down = value_entry_leg(
        model=model,
        leg=ZGapLeg.DOWN,
        book=LegBookView(ask=Decimal("0.14"), ask_depth=Decimal("20"), bid=None, bid_depth=None),
        config=cfg,
    )
    ...
    up2 = value_entry_leg(
        model=model,
        leg=ZGapLeg.UP,
        book=LegBookView(ask=Decimal("0.40"), ask_depth=Decimal("20"), bid=None, bid_depth=None),
        config=cfg,
    )
...
def test_both_legs_edge_and_threshold() -> None:
    ...
    up = value_entry_leg(
        model=model,
        leg=ZGapLeg.UP,
        book=LegBookView(ask=Decimal("0.50"), ask_depth=Decimal("5"), bid=None, bid_depth=None),
        config=cfg,
    )
    down = value_entry_leg(
        model=model,
        leg=ZGapLeg.DOWN,
        book=LegBookView(ask=Decimal("0.05"), ask_depth=Decimal("5"), bid=None, bid_depth=None),
        config=cfg,
    )
    sel = select_leg(up, down, config=cfg)
    ...
    cfg2 = replace(
        cfg,
        entry=replace(cfg.entry, theta_take=Decimal("0.01"), reject_both_legs_edge=True),
    )
    model2 = _model(p_up=0.6, p_down=0.4)
    up_b = value_entry_leg(
        model=model2,
        leg=ZGapLeg.UP,
        book=LegBookView(ask=Decimal("0.30"), ask_depth=Decimal("5"), bid=None, bid_depth=None),
        config=cfg2,
    )
    down_b = value_entry_leg(
        model=model2,
        leg=ZGapLeg.DOWN,
        book=LegBookView(ask=Decimal("0.20"), ask_depth=Decimal("5"), bid=None, bid_depth=None),
        config=cfg2,
    )
...
def test_position_valuation_sell_signs_and_no_double_slip() -> None:
    ...
    val = value_position(
        model=model,
        position=pos,
        book=LegBookView(ask=None, ask_depth=None, bid=bid, bid_depth=Decimal("10"), ready=True),
        config=cfg,
    )
    assert val.ready
    ...
    bad = value_position(
        ...
        book=LegBookView(ask=None, ask_depth=None, bid=bid, bid_depth=Decimal("1"), ready=True),
        config=cfg,
    )
...
def test_readiness_gates() -> None:
    ...
    ok = evaluate_readiness(model=model, ptb=ptb, basis=basis, time_ready=True, config=cfg)
    assert ok.ready
    bad_time = evaluate_readiness(model=model, ptb=ptb, basis=basis, time_ready=False, config=cfg)
    assert bad_time.action is StrategyAction.WAIT


def test_readiness_preserves_all_independent_blockers() -> None:
    from dataclasses import replace
    from datetime import timedelta

    cfg = replace(
        ZGapConfig(),
        ptb_time_quality=replace(ZGapConfig().ptb_time_quality, max_ptb_lag_ms=5_000),
    )
    ptb = make_fixture_ptb(
        market_id=MID,
        window_id="w1",
    ...
    result = evaluate_readiness(
        ...
        basis=compute_basis_bps(trading_ref="100", settlement_ref="100"),
        time_ready=False,
        time_evidence={"reason_code": "time_uncertainty_exceeded"},
        config=cfg,
    )
    assert result.reason_code is ZGapReason.TIME_NOT_READY
    assert result.blockers == (ZGapReason.TIME_NOT_READY, ZGapReason.PTB_NOT_USABLE)
    assert result.evidence["blockers"] == ["TIME_NOT_READY", "PTB_NOT_USABLE"]
