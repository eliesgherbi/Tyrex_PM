...
def test_ewma_jump_guard_and_seed() -> None:
    ...
    prior_sigma = est.snapshot().sigma
    ...
    # Jump still updates variance so the estimator can recover.
    assert jumped.sigma is not None and prior_sigma is not None
    assert jumped.sigma > prior_sigma
...
def test_ewma_micro_sigma_does_not_permanent_jump_lockout() -> None:
    """Reproduce the live tiny-σ lockout and prove recovery with floor + update."""
    est = EwmaVolatilityEstimator(
        SigmaConfig(
            min_samples_s=5.0,
            sample_interval_s=1.0,
    ...
    # Quiet warmup with microscopic ticks (historically installed σ≈0).
    for i in range(8):
        px = Decimal("63977.714") + Decimal(i) * Decimal("0.0000001")
        est.update(px, TS0 + timedelta(seconds=i))
    warm = est.snapshot()
    assert warm.sigma is not None
    assert warm.sigma >= 1e-4
    assert warm.ready

    # Ordinary BTC-scale move must not freeze readiness forever.
    jump = est.update(Decimal("63980.0"), TS0 + timedelta(seconds=9))
    assert jump.sigma is not None and jump.sigma >= 1e-4
    # Whether or not this exact tick trips, later samples must become ready.
    recovered = None
    for i in range(10, 25):
        px = Decimal("63980.0") + Decimal(i - 9) * Decimal("0.05")
        recovered = est.update(px, TS0 + timedelta(seconds=i))
        if recovered.ready and not recovered.jump_guard_tripped:
            break
    assert recovered is not None
    assert recovered.ready
    assert not recovered.jump_guard_tripped
    assert recovered.sigma is not None and recovered.sigma >= 1e-4


def test_ewma_zero_returns_do_not_install_zero_sigma() -> None:
    est = EwmaVolatilityEstimator(
        SigmaConfig(min_samples_s=3.0, sample_interval_s=1.0, sigma_floor=1e-4)
    )
    for i in range(6):
        snap = est.update(Decimal("100"), TS0 + timedelta(seconds=i))
    assert snap.sigma is None
    assert not snap.ready
    assert snap.reject_reason == "warming_up"
    moved = est.update(Decimal("100.2"), TS0 + timedelta(seconds=6))
    assert moved.sigma is not None
    assert moved.sigma >= 1e-4
...
def test_fair_value_complement_and_monotonicity() -> None:
    low = compute_fair_value(
        FairValueInput(S=Decimal("95"), K=Decimal("100"), sigma=0.01, tau_s=100, snapshot_ts=TS0)
    )
    high = compute_fair_value(
        FairValueInput(S=Decimal("110"), K=Decimal("100"), sigma=0.01, tau_s=100, snapshot_ts=TS0)
    )
...
def test_fair_value_tau_floor_and_extreme_z() -> None:
    ...
    assert out.model_status == "ready"
    oracle = oracle_fair_value(S="110", K="100", sigma=0.01, tau_s=0.1, tau_floor_s=1.0)
...
def test_oracle_does_not_import_indicators() -> None:
    ...
    path = Path(__file__).resolve().parents[3] / "oracles" / "binary_fair_value_oracle.py"
...
def test_basis_sign_units_freshness() -> None:
    ...
    stale = compute_basis_bps(trading_ref="101", settlement_ref="100", settlement_ref_fresh=False)
