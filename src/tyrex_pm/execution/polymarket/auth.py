...
def derive_signer_address(private_key: str) -> str:
    ...
    try:
        ...
        raise CredentialError(f"private_key_derives_valid_signer=false ({type(exc).__name__})") from None
...
def build_identity_mapping_report(env: Mapping[str, str] | None = None) -> IdentityMappingReport:
    ...
    # Legacy mistaken alias used by pre-R6D loader — detect misuse only
    legacy_poly_addr = _strip_env(source.get("POLYMARKET_ADDRESS")) or funder
