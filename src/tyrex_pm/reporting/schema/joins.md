...
| `risk_decision` | `run_id`, `correlation_id` |
...
| `account_snapshot` | `run_id`, `account_snapshot_seq` |
...
Legacy py-clob: use `external_order_id` on `execution_outcome`; Nautilus order events may be absent — set `data_quality.legacy_execution_truth=partial`.
