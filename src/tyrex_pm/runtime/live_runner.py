"""Unified live public-data runner. OMS dispatch is the only mode fork."""
...
async def run_live(config: ObserveConfig, *, enable_oms: bool = False) -> ObserveRunResult:
    """Shared live adapter loop for observe (dry) and shadow (OMS) modes."""
    if config.mode is not SourceMode.LIVE:
        raise ValueError("run_live requires live mode")
    ...
    if enable_oms:
        if config.shadow is None or not config.shadow.enable_oms:
            raise ValueError("enable_oms requires shadow.enable_oms=true")
        host: ObserveHost = ShadowHost(config, clock=SystemClock())
    else:
    ...
    start_payload: dict = {
        ...
        "shadow_oms": enable_oms,
        "config_fingerprint": config.fingerprint(),
    }
    if enable_oms and config.shadow is not None:
    ...
    try:
            ...
            if host.lifecycle.state.value == "FLAT":
                ...
                host._emit(
                    "terminal_deferred_residual_exposure",
                    {
                        ...
                        "retry": host.retry.snapshot(),
                        "note": "nonzero/pending exposure not forced TERMINAL",
                    },
                )
            host._maybe_persist()
        stop_payload = {
        ...
        host._emit("runtime_stop", stop_payload)
        host.sink.flush()
        host.close()
