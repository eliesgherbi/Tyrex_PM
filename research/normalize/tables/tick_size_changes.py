"""tick_size_changes table rows (M2B.3-A)."""
...
def rows_from_event(date: str, market_id: str, event: MarketEvent) -> list[dict[str, Any]]:
    if event.event_type != EventType.TICK_SIZE_CHANGE:
        return []
    ...
    return [
        {
            ...
            "old_tick_size": str(raw.get("old_tick_size")) if raw.get("old_tick_size") is not None else None,
            "new_tick_size": str(raw.get("new_tick_size")) if raw.get("new_tick_size") is not None else None,
