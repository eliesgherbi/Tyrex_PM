...
**V2 migration progress:** see [V2_migration_plan.md](V2_migration_plan.md) for the canonical phase table and live-attest evidence.

| Layer | State |
|-------|-------|
| Venue SDK | `py-clob-client-v2` (V1 SDK fully removed; enforced by `tests/test_v2_import_isolation.py`). |
| Default CLOB host | `https://clob-v2.polymarket.com` (V2 staging; flipped to `https://clob.polymarket.com` on cutover day). |
...
| Live attestation | `tyrex-pm live-attest` round-trips submit→ack→cancel against `clob-v2.polymarket.com` (verified 2026-04-19) and emits four V2 evidence facts (`v2_environment`, `collateral_check`, `market_info`, plus `tick_quantize_*` on `oms_submit` and `outcome_validation` on `complete`). |
