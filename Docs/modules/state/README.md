...
| `market_store.py` | `ingestion/market_stream` (WS primary) + `runtime/market_data_runtime` (REST bootstrap) | **`MarketStateStore`**: per-token book — best bid/ask, spread, mid, staleness, `reconnect_gap`, executable depth. **Phase 2:** WS-authoritative for paired-binary live; **Phase 1 survival** reads same store for exit planning |
